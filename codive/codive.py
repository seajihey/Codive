    ...
    try:
        ...
        response = openai.ChatCompletion.create(
            ...
            messages=[
                ...
                {"role": "user", "content": (
f"{request.problem}이문제에대한 답으로 다음과 같은 코드를 작성했어. {request.answer}이 코드를 실행시켰을 때 테스트케이스 통과 여부, 시간복잡도, 코드스타일, 실행시간과 메모리사용량을적어줘. 간결하게 다른거 필요없고5개에대한 답만 dic형식으로 주면 되는데, 시간복잡도는 빅오표기로 계산해서주고 코드스타일은 PeP8기준으로 어떠어떠한 스타일이다 어떻게 개선하면좋다 라고 짧게 간결하게 핵심만 ~~함,~~음으로 답변해줘 최대2줄로 해주고 실행시간은 ms로, 메모리사용량은 kb로 해줘 실행시간과 메모리 사용량은 실행환경에 따라 다르겠지만 네가 측정해줘 key값은 모두 영어로 test_pass,time_complexity,code_style,execution_time,memory_usage로 부탁행 value는 테스트케이스통과여부 줄때 통과 , 통과x로 주고 코드스타일과 한글로해주고 나머진 영어로주고 설명은 필요없어 절대 다른말붙이지말고 딕셔너리형태로 줘야해 실제환경과다를수있다 이런말도하지말고 그냥 네가 실행해보고 제일큰 결과만 줘. 절대 설명 붙이지말고 5개에 대한 key와 value로만 이루어진 딕셔너리형태로 반환해"  )}
            ],
